...
function OptimizePage() {
  ...
  useEffect(() => {
    ...
    const fetchOptimizedCode = async () => {
      ...
      setOptimizedCode("");
      try {
        const res = await fetch("http://localhost:8080/ai/optimize", {
          method: "POST",
        ...
        setOptimizedCode("⚠️ Failed to optimize code. Please check your connection or try later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className={`w-screen h-screen flex flex-col ${bg} ${textColor}`}>
      <header className={`w-full h-16 ${theme === "dark" ? "bg-[#161b22] border-b border-[#30363d]" : "bg-white/90 backdrop-blur-xl border-b border-gray-200"} flex items-center px-6`}>
        ...
        <h1 className="ml-4 text-xl font-semibold tracking-tight">Optimized Code</h1>
      </header>

      <main className="flex-1 p-6 overflow-auto">
        {loading ? (
          <div className="flex items-center justify-center h-full text-sm font-medium">Loading AI optimized code...</div>
        ) : (
